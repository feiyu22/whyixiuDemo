...
<body class="gray-bg">
	                                ...
	                                <label class="col-sm-1 control-label">操作类型</label>
	                                 <div class="col-sm-3 m-b-xs">
		                                <div data-toggle="buttons" class="btn-group">
		                                    <label class="btn btn-white  radio active" val="1">
		                                        <input type="radio" id="option1"   name="status" value="1">入库</label>
		                                    <label class="btn btn-white  radio" val="2" id="export">
	...
	<script>
        $(document).ready(function () {
                 ...
                 var e = event || window.event || arguments.callee.caller.arguments[0];
                  if(document.getElementById('serchCode').value.length!=0&&e && e.keyCode==13&&($("#serchCode").is(":focus"))){ // enter 键
                	  if($.inArray($("#serchCode").val(), list)=='-1'){
	                	  ...
	                	  $("form[name='add']:last").attr("id", $("#serchCode").val());
	                	  $("form[name='add']:last").attr("target", $("#serchCode").val());
	                	  $(".stock:last").after("<form name ='add' method ='post' action ='doAddOrEdit' > <div class ='form-group'> "
	            		...
	            		+ "required ='required' > <div id ='select' > </div> </div > <label class ='col-sm-1 control-label'> "
	            		+ "编码 </label> <div class ='col-sm-2'> <input type ='text' name ='code' id ='code' "
	            		+ "class ='form-control' placeholder ='请输入编码' readonly ='readonly'> </div> <label class ='col-sm-1 "
	            		+ "control-label' >条形码</label > <div class ='col-sm-3' > <input type ='text' name ='barcode' "
	            		+ "id ='barcode' class ='form-control' placeholder ='请输入条形码' readonly ='readonly' > </div> </div> "
	            		+ "<div class ='form-group' > <label class ='col-sm-2 control-label' >采购单位 </label >"
	            		+ " <div class ='col-sm-2' > <input type ='text' name ='company' id ='company' class ='form-control'"
	            		...
	            		+ "'form-control' placeholder ='请输入单价'> <div id ='select'> </div> </div> </div> </form >"
	            		+ "<div class='hr-line-dashed stock'></div>"
        	...
        	$("#company").bind("input propertychange" ,function(e){
        		$("input[name='company']").each(function() {
    				$(this).val($("#company").val().trim());
    			});
        	});
        	
        	$("#price").bind("input propertychange" ,function(e){
        		$("input[name='price']").each(function() {
    				$(this).val($("#price").val().trim());
    			});
        	});
        	
        	//跳转出库操作
        	$("#export").click(function(){
        		window.location.href="/stock/exitus";
        	});
